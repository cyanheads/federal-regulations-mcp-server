...
src/
  ...
  services/
    federal-register/                   # Federal Register API v1 client (keyless)
    ecfr/                               # eCFR API client (keyless) — versioner, structure, search, XML parsing
    ecfr-mirror/                        # eCFR codified-text mirror (MirrorService — SQLite + FTS5)
    regulations-gov/                    # Regulations.gov v4 client (X-Api-Key) — dockets, comments
  mcp-server/
    tools/definitions/                  # 7 regulations_* tools (search-rules, get-document, browse-cfr,
                                        #   get-cfr-section, get-docket, find-comments, list-open-comments)
    resources/definitions/              # 2 resources (document, cfr-section)
scripts/
  ecfr-mirror-init.ts                   # out-of-band mirror lifecycle (init / refresh / verify)
  ecfr-mirror-refresh.ts
  ecfr-mirror-verify.ts
  _mirror-context.ts                    # shared shim imported by the three mirror scripts
```
...
| `npm run lint:mcp` | Validate MCP tool/resource definitions against the linter rule catalog |
| `npm run lint:packaging` | Verify `server.json` ↔ `manifest.json` env var names match (used by devcheck) |
| `npm run mirror:init` | Build the eCFR codified-text mirror (out-of-band, idempotent, resumable). Scope with `ECFR_MIRROR_TITLES`. |
| `npm run mirror:refresh` | Incremental refresh of the eCFR mirror against the latest eCFR issues |
| `npm run mirror:verify` | Report mirror row counts and the last-synced issue date |
