...
**Coverage is per title, and so is freshness.** The in-process refresh is opt-in, so a mirror built once keeps serving that issue's rows as current: a June mirror answered 14 CFR 1.1 with the light-sport aircraft definition a July amendment removed, and 14 CFR 1216.102 after the August issue withdrew it. So every current read also asks whether each title it would answer from is at that title's latest issue: the title's `cfr_part_index.issue_date` (what the ingest stamped on its rows) against its `latest_issue_date` in the titles document `EcfrService` already caches for 15 minutes — per request, with no upstream request beyond that cached read. An older title is read live: section reads (tool and resource) and title-scoped searches, and an all-titles search uses the mirror only when it is complete and every title in it is current. *Decision:* the per-title date, not the sync `checkpoint`, which is the latest date any title reached — an index checkpointed 2026-06-08 held title 1 rows from 2024-05-17. *Decision:* the ingest rewrites a title's `cfr_part_index` rows only after the runner has committed its section rows, so a run aborted or failed between reading a newer issue and writing it leaves the index at the older date: the date may trail the rows (a live read) but never lead them (older text served as current). *Decision:* `latest_issue_date`, not `latest_amended_on`, because an issue can change a section under an old amendment date (title 1's 2026-08-10 issue changed § 17.2 while `latest_amended_on` stayed 2022-12-29), and it is the value both the ingester and the live path's "current" read use. A title the titles document does not name counts as not current, and a titles document that cannot be read fails the read as the live path would, since every current read needs it. A mirror search reports `date` as the titles document's index date, the value a current live search reports: a mirror that answers only for titles at their latest issue holds exactly the text in effect that day, and `sourceScope` names it. Without a refresh, a whole-Code mirror therefore sends most all-titles searches live, since eCFR re-issues some title most business days.
...
cfr_part: z.string().optional()                         // FR conditions[cfr][part]: a part or a range ("140-143"); Part/pt. prefix dropped (normalizePart in services/ecfr/cite.ts, shared with browse_cfr and get_cfr_section); needs cfr_title
...
part: z.string().optional()
  .describe('CFR part within the title, in both modes — structure mode lists every section and appendix in the part, flattened and paged by page/per_page; search mode restricts matches to text inside that part. Requires title; a part on its own is rejected. Parts can be alphanumeric ("1203a", "16A") and are matched exactly once a leading "Part" or "Pt." is dropped, so pass the identifier as eCFR writes it — "58" ("Part 58" also reads as 58), not "058".'),
...
```ts
{
  ...
  nodes: Array<{                      // the 50 titles; a title's direct children; or one page of a part's leaves
    ...
    reserved: boolean,
    cfrCite: string | null,           // → regulations_get_cfr_section: "40 CFR 50.1" (section; "14 CFR 241 § 01" for one
                                      //   numbered without its part), "Appendix A-1 to Part 50, Title 40" (appendix);
                                      //   null on a level with no read path
...
**A section cites the way the read tool cites it.** A part listing knows the part, so a section node cites through `sectionCite`: `40 CFR 141.61` when the identifier carries its part, `14 CFR 241 § 01` when it does not. A bare `${title} CFR ${identifier}` cited 14 CFR 241's sections as `14 CFR 01`, a string that names a different part and matched neither the read tool's `cfrCite` nor a search hit's.

**One structure read per title and date.** `EcfrService` holds the parsed structure document keyed by title and resolved date, so consecutive pages of a part, and the parts of one title, cost one upstream read: walking 40 CFR 63's 3,120 leaves at `per_page` 50 was 63 downloads of the same 9.4 MB document (941 KB gzipped, the largest of the 49 non-reserved titles). An undated browse resolves its date from the cached titles document, so a title's new issue changes the key and is read fresh — the same `latest_issue_date` signal the mirror's freshness check reads. Entries lapse after the titles document's 15-minute horizon, at most 4 are held (oldest out first), and a failed read or a 404 is never stored. *Decision:* the whole document rather than one part's flattened listing, because browsing a second part of the same title would otherwise download it again.
...
```ts
{
  ...
  date: string,                       // the day whose text was searched — on the mirror too, the titles
                                      //   document's index date, since it answers only for current titles
  results: Array<{
    title: number,
    part: string,
    section: string | null,
...
| Reason | Code | When | Recovery |
|:-------|:-----|:-----|:---------|
| `query_required` | `ValidationError` | `mode='search'` with no `query` | Provide a `query` phrase for search mode, or switch to `mode='structure'` to browse. |
...
| `title_required_for_part` | `ValidationError` | `part` given with no `title`, either mode | Add the title the part belongs to (e.g. title 40 with part 58), or drop `part`. |
| `date_out_of_range` | `ValidationError` | Structure mode, `date` past the title's `up_to_date_as_of`; search mode, `date` before 2017-01-03 or past the current index date | Pick a date inside the window the error names, or omit `date` to browse or search the current text. |
...
Two ways a block tag is not what it looks like, both of which the extractor has to name. A block ends at the first closer of its own name, so a variant tag name has to be matched in full: reading `<FP-2>` as `FP` runs on to the next `</FP>`. And the same family is written self-closing where it stands for spacing or a rule rather than for text — `<PSPACE/>`, `<FP-DASH/>`, `<P/>` — which must not open a block at all, for the same reason in the other direction: it would run on to the next closing tag of that name and take the blocks between with it, flattening their paragraph breaks and dropping any figure among them. Across five whole titles the self-closing shape reaches 6 nodes, all in Title 40 — rare enough that a sample drawn from smaller titles reads as though it does not occur.

**Every scan is linear, malformed input included.** A tag never spans another `<` — the rule `stripHtml` already follows — and a block's end is a lookup in a one-pass index of closer positions (`CloserIndex` in `src/services/ecfr/xml.ts`) whose per-name cursor only moves forward, rather than a lazy search from each opener. The old patterns rescanned to the end from every `<` with no `>` and every block opener with no closer, so 16× the input cost ~250× the time. Versioner XML escapes a literal `<`, so this changed no well-formed output: old and new extraction were byte-identical across 29 real documents (174.5 M characters, 17,796 sections and appendices). Search excerpts do not escape it — 40 CFR 142.62's reads `systems <500 <strong>service</strong> connections`, which the old label pattern cut to `systems service connections`. Rendering costs ~25% more parse time over the same sample (1.9 s → 2.4 s), against a whole-title fetch measured in minutes.

**Inline markup renders instead of flattening.** Superscripts (`<sup>`, `<SU>`, `<E T="51">`/`"53"`) read `^x` and subscripts (`<sub>`, `<E T="52">`/`"54"`) `_x`, braced when the run holds a space or a letter or digit follows it (`CO_{2}e`), bare otherwise (`10^−8`, `SO_2`). *Decision:* ASCII rather than Unicode super/subscript characters — Unicode has no subscript for most letters, capitals, or `.`, and the mirror's `unicode61` tokenizer treats `₂` as a token character, so `PM₂.₅` would stop matching `pm 2`. A sign-only run (`−`, `+`, `±`) joins the same-kind run beside it across whitespace (`3 × 10<sup>−</sup> <sup>8</sup>` → `3 × 10^−8`) and nothing else merges, so two footnote letters `^d ^e` stay two. `[n]` marks a footnote only on a structural signal — `<SU>` before `<FTREF/>`, or `<SU>` opening a paragraph — because a table's `<sup>1</sup>` marker is indistinguishable from an exponent. Inline elements (`<I>`, `<B>`, `<E>`, `<em>`, `<strong>`, the runs, `<AC>`, `<FTREF>`) add no separator, so `Paragraph (<E T="01">a</E>)` reads `(a)` rather than `( a)`; every other tag is still a space (`MCL<br></br>(mg/l)` → `MCL (mg/l)`, `8<FR>1/2</FR>` → `8 1/2`). The Code nests no run inside another (none in 166 MB sampled), so a run opened inside a run reads as the outer one's text. Structure and search labels are left as plain text: section headings write the same `PM10` with an `<E>` code that is not a subscript, and marking only one surface would make browse and read headings disagree.

**Diacritics and overlines are combining marks.** `<AC T="…"/>` follows the character it modifies, almost always across a newline (`<I>x⏎<AC T="8"/></I>`), and renders as the mark(s) eCFR's own renderer prints after the base, with the whitespace between them dropped — across a closing `</I>` or `</E>` too — so the section reads `x̄ is the sample mean`. The code table (`b` dot above, `8` macron, `i` dot then overline, `3` circumflex, `g` macron below, `6` tilde, `j` dot then tilde, `2` grave; `1` `4` `7` `9` from GPO's XML guide) follows the renderer, which overrides the guide's `~` for code `8`; any other code, including the `I` the renderer prints nothing for, adds no mark. `<E T="7503">` is an overline span, so each character in it takes U+0305 (`R̅M̅`, "Average RM value"). FTS tokens do not change (`remove_diacritics 2` indexes `x̄` as `x`), so a search for `x` or `x̄` still finds the row.

**A whole part carries its heading, Authority, Source, and notes.** A part's `<DIV5>` preamble holds its `<HEAD>` (`PART 141—NATIONAL PRIMARY DRINKING WATER REGULATIONS`), its `<AUTH>` and `<SOURCE>`, and part-level `<EDNOTE>` / `<NOTE>` / `<APPRO>`; none sits inside a section, so no read returned them. A whole-part read now returns them as `heading`, `authority`, `sourceNote` (labels dropped; the Source is named `sourceNote` because `source` already names the answering corpus), and `notes`. A subpart or subject group states its own Authority or Source for the sections under it — 10 CFR 20 has no part-level Source, and each of its subparts carries one — so those ride on the `sections[]` entries they govern, the nearest level winning for each of the two independently. A part-level note written after the part's first subpart (10 CFR 205 carries a Source after Subpart W) governs what follows it rather than the whole part. *Decision:* single-section and appendix reads carry none of these, on either path. A section-filtered versioner response is a bare `<DIV8>` with no preamble, and fetching the part to decorate one section costs ~61× the bytes (40 CFR 141: 1.9 MB vs 31 KB); storing the notes in the mirror would answer the same cite with or without its Source depending on which corpus replied — the split the marker-3 bump exists to prevent. A section's own `<SECAUTH>` (its statutory authority) and `<APPRO>` (its OMB control number) are different: they sit inside the `<DIV8>`, and stay in its `bodyText` in document order, ahead of the `<CITA>`.

Across 23 sampled documents (titles 1, 10, 11 and 20 parts; 16,622 sections) the rendering, the marks, and the section notes change the stored text of 24% of sections, which is why the ingest marker moves to 4.
...
part: z.string().optional()
  .describe('CFR part within the title (e.g. "50"). Parts can be alphanumeric; a leading "Part" or "Pt." is dropped. Required unless appendix is given, where it is optional but recommended. Obtain from regulations_browse_cfr or from a Federal Register document\'s cfrReferences.'),
section: z.union([z.literal(''), z.string()]).optional()
  .describe('Section within the part, normally part.section ("141.61"); "61", "§ 141.61", "Sec. 141.61", "141.61(c)", and the full cite this tool returns ("40 CFR 141.61", "14 CFR 241 § 25") also resolve. Omit to fetch the entire part. Cannot be combined with appendix.'),
...
```ts
{
  ...
  heading: string,                    // "§ 50.1 Definitions."; a whole part's own "PART 141—…"
  ...
  authority?: string | null,          // whole part only: the part's Authority, label dropped; null when none
  sourceNote?: string | null,         // whole part only: the part's Source note, label dropped; null when none
  notes?: string[],                   // whole part only: part-level EDNOTE / NOTE / APPRO text
  bodyText: string,                   // one window of the text, XML stripped to plain text; paragraphs, HD
                                      //   subheadings, editorial notes, tables (pipe-delimited rows), a section's
                                      //   own SECAUTH / APPRO, the trailing <CITA> source citation, and figure
                                      //   references kept in document order; ^x / _x runs, [n] footnote markers,
                                      //   combining marks for diacritics and overlines
  ...
  sections?: Array<{                  // whole part only: the sections whose text falls in this window
    ...
    authority?: string;               //   a subpart's or subject group's own; absent when the part's applies
    sourceNote?: string;              //   likewise
  }>,
...
`format()`: header (cite, heading, hierarchy path, effective date, `source`), a whole part's Authority, Source, and notes, the window's span and resume offset, the section index and appendix handles for a part, then the window's text last and untrimmed so consecutive windows rebuild the body from `content[]` as exactly as from `structuredContent`.
...
**The cites this server writes read back.** Every `cfrCite` sits beside "→ regulations_get_cfr_section", so `40 CFR 141.61`, `40 C.F.R. 141.61`, and the dotless `14 CFR 241 § 25` all resolve as `section`: a leading `<title> CFR ` or `<title> C.F.R. ` naming the call's `title` is stripped together with a following `<part> §` naming its part, before the first lookup and at no extra lookup — none of the 59,771 section identifiers in titles 7, 14, 40, and 48 contains `CFR` or begins with digits and a space. A cite naming another title fails `conflicting_title` before any request, naming both numbers. `part` goes through the `normalizePart` that `regulations_browse_cfr` and `regulations_search_rules`' `cfr_part` share (a leading `Part` / `Pt.`, surrounding whitespace), moved to `src/services/ecfr/cite.ts` so the resource uses it too, and the `notice` names each rewrite. *Decision:* a new `conflicting_title` rather than `conflicting_target`, whose recovery (send section or appendix, not both) is wrong for a title clash.
...
| `location_required` | `ValidationError` | Neither `part` nor `appendix` given | Add the part to read, or the appendix identifier from regulations_browse_cfr. |
| `conflicting_target` | `ValidationError` | Both `section` and `appendix` given | Send one or the other; make two calls to read both. |
| `conflicting_title` | `ValidationError` | `section` is a full cite naming another title (`"21 CFR 141.61"` with `title` 40) | Set title to the title the cite names, or pass the section without its title prefix. |
| `date_out_of_range` | `ValidationError` | `date` precedes eCFR historical coverage (2017-01-01) or is past the title's up-to-date date | Use a date inside the window the error names, or omit `date` for the current text. |
